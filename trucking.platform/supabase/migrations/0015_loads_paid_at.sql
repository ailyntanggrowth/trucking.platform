-- Módulo 2 — guarda la fecha en que una carga pasó a "Pagada". La comisión
-- del despachador (lib/settlements.ts, dispatcherCommissionDetail) agrupa por
-- esta fecha, no por la fecha de recogida: una carga que Summar paga tarde
-- simplemente cae en el invoice de la semana en que sí se pague (pedido
-- explícito de la dueña). Aplicar manualmente en el SQL Editor de Supabase,
...
begin
  ...
  insert into loads (
    ...
    p_load->>'payment_status', (p_load->>'amount_received')::numeric, nullif(p_load->>'paid_at','')::date, p_load->>'notes', now()
  )
...
begin
  ...
  insert into loads (
    ...
    p_replacement->>'payment_status', (p_replacement->>'amount_received')::numeric, nullif(p_replacement->>'paid_at','')::date, p_replacement->>'notes',
    p_original_id, now()
  );
