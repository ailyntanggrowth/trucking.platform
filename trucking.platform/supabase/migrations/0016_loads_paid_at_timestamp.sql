-- Módulo 2 — corrige loads.paid_at de "date" a "timestamptz". La regla real
-- (pedido explícito de la dueña) es que el lunes de cierre reparte cargas
-- entre DOS invoices según la HORA en que se pagaron, no solo la fecha: lo
-- pagado el lunes ANTES de las 9pm cae en el invoice que cierra esa noche;
-- lo pagado el lunes DESPUÉS de las 9pm ya es del invoice de la semana
-- siguiente. Con solo la fecha (sin hora) eso no se puede distinguir.
-- Aplicar manualmente en el SQL Editor de Supabase, igual que las anteriores.

alter table loads alter column paid_at type timestamptz using paid_at::timestamptz;
...
begin
  ...
  insert into loads (
    ...
    amount, status, missing_pod, payment_status, amount_received, paid_at, notes, updated_at
  ) values (
    ...
    p_load->>'payment_status', (p_load->>'amount_received')::numeric, nullif(p_load->>'paid_at','')::timestamptz, p_load->>'notes', now()
  )
  on conflict (id) do update set
    ...
    amount_received = excluded.amount_received, paid_at = excluded.paid_at, notes = excluded.notes, updated_at = now();
...
begin
  ...
  insert into loads (
    id, company_id, load_number, broker, driver_id, truck_id, trailer_id,
    pickup_city, pickup_state, pickup_date, delivery_city, delivery_state, delivery_date,
    amount, status, missing_pod, payment_status, amount_received, paid_at, notes, replaces_id, updated_at
  ) values (
