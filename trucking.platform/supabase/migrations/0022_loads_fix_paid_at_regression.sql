-- Corrige un error mío: la migración 0021 (que arregló la aprobación
-- automática al crear una carga) sin querer se olvidó de paid_at por
-- completo — ni lo insertaba en cargas nuevas ni lo actualizaba al editar
-- una carga existente, así que marcar una carga como pagada dejó de anotar
-- la fecha/hora del pago (se quedaba en null), rompiendo el invoice de la
-- despachadora para cualquier carga tocada después de esa migración.
-- Esta versión trae de vuelta paid_at (insert Y update) y conserva el
-- arreglo de approval (solo al insertar, nunca al editar).
