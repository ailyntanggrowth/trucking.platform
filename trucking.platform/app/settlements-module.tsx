...
export default function SettlementsModule({ settlements, loads, fuel, fleet, lang, t }: {
  ...
  return <div className={styles.settlements}>
    ...
    {tab === 'ownerOperators' && <>
      <p className={styles.note}>{t('Reporte angosto (spec 9.10): solo el bruto y el combustible de cada Owner Operator, para calcular el 12% de Mario y el reembolso de combustible. No es una liquidación completa.')}</p>
      <div className={styles.cards}>{ownerOperators.map(o => <article className={styles.card} key={o.driverId}>
        ...
        <p><b>{t('Combustible a reembolsar:')}</b> {money(o.fuel)}</p>
      </article>)}</div>
