...
export default function FleetModule({fleet,loads,onOpenLoads,lang,t,initialTab}:{fleet:FleetController;loads:Load[];onOpenLoads:()=>void;lang:Lang;t:(es:string)=>string;initialTab?:Tab}) {
  ...
  // Choferes y Flota es solo para Mario + Owner Operators (spec 7.3a). Lázaro,
  // Dionisio y sueltos como Abraham existen en la base de datos únicamente
  // para que Combustible pueda separar su gasto de fuel — no deben aparecer
  // aquí ni contarse en las tarjetas/pestañas de este módulo.
  const scopedDrivers=state.drivers.filter(d=>d.group==='Mario'||d.group==='Owner Operators');
  const records=tab==='assignments'||tab==='actividad'?[]:(tab==='drivers'?scopedDrivers:state[tab]).filter(r=>{const search=tab==='drivers'?`${(r as Driver).name} ${(r as Driver).phone} ${(r as Driver).email} ${(r as Driver).group}`:`${(r as Equipment).unit} ${(r as Equipment).vin} ${(r as Equipment).plate} ${(r as Equipment).make}`;return search.toLocaleLowerCase().includes(query.toLocaleLowerCase())&&(filter==='Todos'||statusOf(tab,r.id)===filter);});
  ...
  return <div className={styles.fleet}>
    ...
    <div className={styles.statCards}>
      <button className={styles.statCard} data-tone="blue" onClick={()=>changeTab('drivers')}><span className={styles.statIcon} aria-hidden="true"><User size={16}/></span><span className={styles.statLabel}>{t('Choferes activos')}</span><strong>{ready?scopedDrivers.filter(d=>d.active).length:'—'}</strong></button>
      <button className={styles.statCard} data-tone="green" onClick={()=>changeTab('drivers')}><span className={styles.statIcon} aria-hidden="true"><CheckCircle2 size={16}/></span><span className={styles.statLabel}>{t('Choferes disponibles')}</span><strong>{ready?scopedDrivers.filter(d=>d.active&&d.availability==='Disponible'&&!assignmentFor(state,'drivers',d.id)).length:'—'}</strong></button>
    ...
    <nav className={styles.tabs} aria-label={t('Secciones de flota')}>{(Object.keys(titles) as Tab[]).map(tabKey=><button key={tabKey} aria-pressed={tab===tabKey} onClick={()=>changeTab(tabKey)}>{t(titles[tabKey])} <span>{tabKey==='assignments'?activeAssignments.length:tabKey==='actividad'?state.events.length:tabKey==='drivers'?scopedDrivers.length:state[tabKey].length}</span></button>)}</nav>
