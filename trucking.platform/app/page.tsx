...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {activeModule===null ? <section className="landingHero" id="main-content" tabIndex={-1}>
      ...
      <section className="panel sectionSpace" id="actividad" tabIndex={-1}><div className="panelHeader"><div><h2>{t('Actividad reciente')}</h2><p>{t('Quién hizo cada cambio y cuándo.')}</p></div></div>{data.activity.length?<><ol className="plainList">{[...data.activity].sort((a,b)=>b.at.localeCompare(a.at)).slice(0,3).map(a=><li key={a.id} className="alertLine"><strong>{a.detail}</strong><span>{dateLabel(a.at)} · {a.actor}</span></li>)}</ol><div className="panelLinkWrap"><button className="selectButton" onClick={()=>openFleet('actividad')}>{t('Ver toda la actividad →')}</button></div></>:<p className="emptyState">{fleetReady?t('Todavía no hay actividad de flota.'):unavailable}</p>}</section>
